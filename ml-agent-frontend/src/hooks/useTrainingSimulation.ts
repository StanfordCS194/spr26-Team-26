...
const ITERATIONS: Array<Omit<Iteration, 'id'>> = [
  {
    ...
    reasoning: {
      hypothesis: 'Training loss showed high variance across batches, suggesting the LR is too large and causing the optimizer to overshoot minima.',
      strategy: 'local_perturbation',
      expectedImpact: 'val_loss should drop by 5–10% and loss variance should fall within 1 epoch.',
  ...
  },
  {
    ...
    reasoning: {
      hypothesis: 'F1 plateau at 0.871 suggests the LoRA adapter lacks representational capacity. Doubling rank adds trainable params without touching the frozen base.',
  ...
  },
  {
    ...
    reasoning: {
      hypothesis: 'Probing upward tests whether the stabilised minima is a saddle point a larger step could escape.',
      strategy: 'claude_proposal',
      expectedImpact: 'Possible 1–2% F1 gain if a better basin exists; risk of divergence if not.',
  ...
  },
  {
    ...
    searchCoord: { lr: 1.5e-4, loraRank: 32 },
    reasoning: {
      hypothesis: 'Early loss spikes suggest LR ramps too quickly. Longer warmup gives the optimizer time to find a stable trajectory before reaching peak LR.',
      strategy: 'local_perturbation',
      expectedImpact: 'Smoother early loss curve and ~1–3% improvement in final val_loss.',
      rationale: 'val_loss dropped 0.271 → 0.248 (−8.5%), F1 improved 0.901 → 0.914. Early spike eliminated.',
  ...
  },
  {
    ...
    searchCoord: { lr: 1.5e-4, loraRank: 32 },
    reasoning: {
      hypothesis: 'Train/val gap is small — model is not overfitting. Current dropout may be over-regularising; reducing it lets the model use more capacity.',
      strategy: 'local_perturbation',
      expectedImpact: 'Minor F1 improvement of ~0.5–1% with negligible change in generalisation gap.',
  ...
  },
  {
    ...
    searchCoord: { lr: 1.5e-4, loraRank: 32 },
    reasoning: {
      hypothesis: 'Val loss was still gently declining at epoch 3 — adding an epoch might extract further signal from the data.',
      strategy: 'random_search',
      expectedImpact: 'F1 improvement of 0.5–1.5% if convergence is genuinely incomplete; risk of overfitting if not.',
      rationale: 'val_loss rose 0.243 → 0.261 (+7.4%) and F1 dropped 0.917 → 0.908 — classic overfitting onset. Reverted.',
  ...
  },
  {
    ...
    reasoning: {
      hypothesis: 'With warmup extended, the effective peak LR may still be slightly high. A conservative nudge probes whether slower late-stage updates improve final convergence.',
      strategy: 'local_perturbation',
      expectedImpact: 'val_loss decrease of 2–5% with no regression in F1.',
      rationale: 'val_loss dropped 0.243 → 0.231 (−4.9%), F1 climbed 0.917 → 0.923. Best config so far — kept.',
...
export function useTrainingSimulation() {
  ...
  const start = useCallback((prompt: string, budget: number, taskType: TaskType, skillLevel: SkillLevel) => {
    ...
    setState({
      ...
      taskType,
      skillLevel,
      capabilities: getCapabilities(skillLevel),
      pendingApproval: null,
      costSpent: 0,
      metrics: [],
      iterations: [],
